...
import 'package:test_browser_2/app_bar/url_info_popup.dart';
import 'package:test_browser_2/custom_image.dart';
import 'package:test_browser_2/main.dart';
import 'package:test_browser_2/models/browser_model.dart';
import 'package:test_browser_2/models/favorite_model.dart';
import 'package:test_browser_2/models/web_archive_model.dart';
import 'package:test_browser_2/models/webview_model.dart';
import 'package:test_browser_2/pages/developers/main.dart';
import 'package:test_browser_2/pages/settings/main.dart';
import 'package:test_browser_2/tab_popup_menu_actions.dart';
...
class _WebViewTabAppBarState extends State<WebViewTabAppBar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _focusNode?.addListener(() async {
      if (_focusNode != null && !_focusNode!.hasFocus && _searchController != null && _searchController!.text.isEmpty) {
        ...
        _searchController!.text = (await webViewController?.getUrl())?.toString() ?? "";
      }
    });
  }
        ...
        builder: (context, url, child) {
          ...
          return Selector<WebViewModel, bool>(
              ...
              builder: (context, isIncognitoMode, child) {
                return leading != null
                    ? AppBar(
                        backgroundColor: isIncognitoMode ? Colors.black87 : Colors.blue,
    ...
    return IconButton(
      ...
      onPressed: () {
        if (webViewController != null) {
          var url = settings.homePageEnabled && settings.customUrlHomePage.isNotEmpty
        ...
        children: <Widget>[
          TextField(
            ...
            decoration: InputDecoration(
              contentPadding: const EdgeInsets.only(left: 45.0, top: 10.0, right: 10.0, bottom: 10.0),
              filled: true,
              ...
              webViewModel.screenshot = await webViewController
                  .takeScreenshot(
                      screenshotConfiguration:
                          ScreenshotConfiguration(compressFormat: CompressFormat.JPEG, quality: 20))
                  .timeout(
            ...
            CustomPopupMenuItem<String>(
              ...
              child: StatefulBuilder(
                builder: (statefulContext, setState) {
                  var browserModel = Provider.of<BrowserModel>(statefulContext, listen: true);
                  ...
                  if (webViewModel.url != null && webViewModel.url!.toString().isNotEmpty) {
                    favorite = FavoriteModel(
                        url: webViewModel.url, title: webViewModel.title ?? "", favicon: webViewModel.favicon);
                  ...
                  children.addAll([
                            ...
                            onPressed: () {
                              setState(() {
                                if (favorite != null) {
                                  ...
                                  } else if (browserModel.containsFavorite(favorite)) {
                    ...
                    SizedBox(
                        ...
                        child: IconButton(
                            ...
                            onPressed: () async {
                              Navigator.pop(popupMenuContext);
                              if (webViewModel.url != null && webViewModel.url!.scheme.startsWith("http")) {
                                ...
                                String? savedPath = (await webViewController?.saveWebArchive(
                                ...
                                if (savedPath != null) {
                                  browserModel.addWebArchive(url.toString(), webArchiveModel);
                                  if (mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                      content: Text("${webViewModel.url} saved offline!"),
                                    ));
                                  }
                                  browserModel.save();
                                } else {
              ...
              content: SizedBox(
                  ...
                  child: ListView(
                    children: browserModel.favorites.map((favorite) {
                      ...
                      var faviconUrl =
                          favorite.favicon != null ? favorite.favicon!.url : WebUri("${url?.origin ?? ""}/favicon.ico");
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(favorite.url?.toString() ?? "", maxLines: 2, overflow: TextOverflow.ellipsis),
                        isThreeLine: true,
                        onTap: () {
                          setState(() {
                              ...
                              return ListTile(
                                leading: Column(
                                  ...
                                  children: <Widget>[
                                    ...
                                    // )
                                    CustomImage(
                                        url: WebUri("${url?.origin ?? ""}/favicon.ico"), maxWidth: 30.0, height: 30.0)
                                  ],
                                ),
                                title: Text(historyItem.title ?? url.toString(),
                                    maxLines: 2, overflow: TextOverflow.ellipsis),
                                subtitle: Text(url?.toString() ?? "", maxLines: 2, overflow: TextOverflow.ellipsis),
                                isThreeLine: true,
                                onTap: () {
            ...
            listViewChildren.add(ListTile(
              leading: Column(
                ...
                children: <Widget>[
                  ...
                  //   height: 30,
                  // )
                  CustomImage(url: WebUri("${url?.origin ?? ""}/favicon.ico"), maxWidth: 30.0, height: 30.0)
                ],
              ),
              title: Text(webArchive.title ?? url?.toString() ?? "", maxLines: 2, overflow: TextOverflow.ellipsis),
              subtitle: Text(url?.toString() ?? "", maxLines: 2, overflow: TextOverflow.ellipsis),
              trailing: IconButton(
  ...
  void goToDevelopersPage() {
    Navigator.push(context, MaterialPageRoute(builder: (context) => const DevelopersPage()));
  }
  ...
  void takeScreenshotAndShow() async {
    ...
    if (screenshot != null) {
      ...
      File file = File("${dir.path}/screenshot_${DateTime.now().microsecondsSinceEpoch}.png");
