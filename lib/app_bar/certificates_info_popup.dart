...
import 'package:test_browser_2/models/webview_model.dart';
...
class _CertificateInfoPopupState extends State<CertificateInfoPopup> {
          ...
          return FutureBuilder(
            ...
            builder: (context, snapshot) {
              ...
              return Center(
                child: Container(
                  decoration:
                      const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(2.5))),
  ...
  Future<void> _getOtherCertificatesFromTopMain(
      List<X509Certificate> otherCertificates, X509Certificate x509certificate) async {
        ...
        children: <Widget>[
          Row(
            ...
            children: <Widget>[
              Container(
                decoration:
                    const BoxDecoration(color: Colors.green, borderRadius: BorderRadius.all(Radius.circular(5.0))),
                                          ...
                                          child: StatefulBuilder(
                                            builder: (context, setState) {
                                              ...
                                              for (var certificate in certificates) {
                                                var name =
                                                    _findCommonName(x509certificate: certificate, isSubject: true) ??
                                                        _findOrganizationName(
                                                            x509certificate: certificate, isSubject: true) ??
                                                        "";
                                                if (name != "") {
  ...
  List<Widget> _buildIssuedToSection(X509Certificate x509certificate) {
    var subjectCountryName =
        _findCountryName(x509certificate: x509certificate, isSubject: true) ?? "<Not Part Of Certificate>";
    var subjectStateOrProvinceName =
        _findStateOrProvinceName(x509certificate: x509certificate, isSubject: true) ?? "<Not Part Of Certificate>";
    var subjectCN = _findCommonName(x509certificate: x509certificate, isSubject: true) ?? "<Not Part Of Certificate>";
    var subjectO =
        _findOrganizationName(x509certificate: x509certificate, isSubject: true) ?? "<Not Part Of Certificate>";
    var subjectU =
        _findOrganizationUnitName(x509certificate: x509certificate, isSubject: true) ?? "<Not Part Of Certificate>";
  ...
  List<Widget> _buildIssuedBySection(X509Certificate x509certificate) {
    var issuerCountryName =
        _findCountryName(x509certificate: x509certificate, isSubject: false) ?? "<Not Part Of Certificate>";
    var issuerStateOrProvinceName =
        _findStateOrProvinceName(x509certificate: x509certificate, isSubject: false) ?? "<Not Part Of Certificate>";
    var issuerCN = _findCommonName(x509certificate: x509certificate, isSubject: false) ?? "<Not Part Of Certificate>";
    var issuerO =
        _findOrganizationName(x509certificate: x509certificate, isSubject: false) ?? "<Not Part Of Certificate>";
    var issuerU =
        _findOrganizationUnitName(x509certificate: x509certificate, isSubject: false) ?? "<Not Part Of Certificate>";
  ...
  List<Widget> _buildKeyUsageSection(X509Certificate x509certificate) {
    ...
    var keyUsageIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.keyUsage.toValue()) != null
            ? "YES"
            : "NO";
          ...
          keyUsageSection.addAll(<Widget>[
            ...
            RichText(
              text: TextSpan(children: [
                ...
                TextSpan(text: keyUsageIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
              ]),
            ),
            RichText(
              text: TextSpan(children: [
                const TextSpan(
                    text: "Usage ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
                TextSpan(
                    text: KeyUsage.fromIndex(i)!.name(), style: const TextStyle(fontSize: 12.0, color: Colors.black))
              ]),
            ),
          ]);
        }
  ...
  List<Widget> _buildBasicConstraints(X509Certificate x509certificate) {
    ...
    var basicConstraintsIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.basicConstraints.toValue()) != null
            ? "YES"
            : "NO";
    if (basicConstraints != null && basicConstraints.pathLenConstraint == -1) {
      basicConstraintsSection.addAll(<Widget>[
        ...
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: basicConstraintsIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
        RichText(
          text: const TextSpan(children: [
            TextSpan(
                ...
                style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: "NO", style: TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
      ]);
    } else {
        ...
        RichText(
          text: const TextSpan(children: [
            TextSpan(
                text: "Certificate Authority ",
                style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: "YES", style: TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                ...
                style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: basicConstraints.toString(), style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
      ]);
    }
  ...
  List<Widget> _buildExtendedKeyUsage(X509Certificate x509certificate) {
    ...
    var extendedKeyUsageIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.extKeyUsage.toValue()) != null
            ? "YES"
            : "NO";
          ...
          RichText(
            text: TextSpan(children: [
              const TextSpan(
                  text: "Critical ",
                  style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
              TextSpan(text: extendedKeyUsageIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
            ]),
          ),
          RichText(
            text: TextSpan(children: [
  ...
  List<Widget> _buildSubjectKeyIdentifier(X509Certificate x509certificate) {
    ...
    var subjectKeyIdentifierIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.subjectKeyIdentifier.toValue()) != null
            ? "YES"
            : "NO";
    if (subjectKeyIdentifier?.value != null && subjectKeyIdentifier!.value!.isNotEmpty) {
      ...
      subjectKeyIdentifierSection.addAll(<Widget>[
        ...
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: subjectKeyIdentifierIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Key ID ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(text: subjectKeyIdentifierToHexValue, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        )
      ]);
    } else {
  ...
  List<Widget> _buildAuthorityKeyIdentifier(X509Certificate x509certificate) {
    ...
    var authorityKeyIdentifierIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.authorityKeyIdentifier.toValue()) !=
                null
            ? "YES"
            : "NO";
    if (authorityKeyIdentifier?.keyIdentifier != null && authorityKeyIdentifier!.keyIdentifier!.isNotEmpty) {
        ...
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(
                text: authorityKeyIdentifierIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Key ID ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(
                text: authorityKeyIdentifierToHexValue, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        )
      ]);
    } else {
  ...
  List<Widget> _buildCertificatePolicies(X509Certificate x509certificate) {
    ...
    var certificatePoliciesIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.extKeyUsage.toValue()) != null
            ? "YES"
            : "NO";
    ...
    var certificatePoliciesSection = <Widget>[
      ...
      RichText(
        text: TextSpan(children: [
          const TextSpan(
              text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
          TextSpan(text: certificatePoliciesIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
        ]),
      ),
    ];

    if (certificatePolicies?.policies != null && certificatePolicies!.policies!.isNotEmpty) {
  ...
  List<Widget> _buildCRLDistributionPoints(X509Certificate x509certificate) {
    ...
    var cRLDistributionPointsIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.cRLDistributionPoints.toValue()) !=
                null
            ? "YES"
            : "NO";
    ...
    var cRLDistributionPointsSection = <Widget>[
      ...
      RichText(
        text: TextSpan(children: [
          const TextSpan(
              text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
          TextSpan(text: cRLDistributionPointsIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
        ]),
      ),
    ];

    if (cRLDistributionPoints?.crls != null && cRLDistributionPoints!.crls!.isNotEmpty) {
      for (var i = 0; i < cRLDistributionPoints.crls!.length; i++) {
        cRLDistributionPointsSection.addAll(<Widget>[
          RichText(
            text: TextSpan(children: [
              const TextSpan(
                  text: "URI ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
              TextSpan(
  ...
  List<Widget> _buildAuthorityInfoAccess(X509Certificate x509certificate) {
    ...
    var authorityInfoAccessIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.authorityInfoAccess.toValue()) != null
            ? "YES"
            : "NO";
    ...
    var authorityInfoAccessSection = <Widget>[
      ...
      RichText(
        text: TextSpan(children: [
          const TextSpan(
              text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
          TextSpan(text: authorityInfoAccessIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
        ]),
      ),
    ];
  ...
  List<Widget> _buildSubjectAlternativeNames(X509Certificate x509certificate) {
    ...
    var subjectAlternativeNamesIsCritical =
        criticalExtensionOIDs.map((e) => e).firstWhereOrNull((oid) => oid == OID.subjectAltName.toValue()) != null
            ? "YES"
            : "NO";
        ...
        RichText(
          text: TextSpan(children: [
            const TextSpan(
                text: "Critical ", style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
            TextSpan(
                text: subjectAlternativeNamesIsCritical, style: const TextStyle(fontSize: 12.0, color: Colors.black))
          ]),
        ),
      ]);
          ...
          RichText(
            text: TextSpan(children: [
              const TextSpan(
                  text: "DNS Name ",
                  style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.black)),
              TextSpan(text: subjectAlternativeName, style: const TextStyle(fontSize: 12.0, color: Colors.black))
            ]),
          ),
        ]);
      }
