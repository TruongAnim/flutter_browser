...
import 'package:test_browser_2/util.dart';
...
class _StorageManagerState extends State<StorageManager> {
  ...
  final HttpAuthCredentialDatabase _httpAuthCredentialDatabase = HttpAuthCredentialDatabase.instance();
              ...
              return DataRow(cells: <DataCell>[
                _buildDataCellEditable(
                    ...
                    onFieldSubmitted: (newValue) async {
                      var updateCookie = await _cookieManager.getCookie(url: url, name: cookie.name);
                      await _cookieManager.deleteCookie(url: url, name: cookie.name);
                      await _cookieManager.setCookie(url: url, name: cookie.name, value: updateCookie?.value ?? "");
                    },
                ...
                _buildDataCellEditable(
                    width: constraints.maxWidth / 3,
                    onFieldSubmitted: (newValue) async {
                      await _cookieManager.setCookie(url: url, name: cookie.name, value: newValue);
                    },
                                ...
                                children: <Widget>[
                                  Expanded(
                                    child: ListTile(
                                      ...
                                      subtitle: Text(_newCookieExpiresDate != null
                                      ...
                                      onTap: () async {
                                        ...
                                        DatePicker.showDateTimePicker(context,
                                            ...
                                            maxTime: DateTime(9999), onConfirm: (expiresDate) {
                                          setState(() {
                            ...
                            child: TextButton(
                              ...
                              onPressed: () async {
                                if (_newCookieFormKey.currentState != null &&
                                    _newCookieFormKey.currentState!.validate()) {
                                  final expiresDate = _newCookieExpiresDate != null
                                      ? _newCookieExpiresDate!.millisecondsSinceEpoch
                                      : null;
      ...
      builder: (context, webViewModel, child) {
        ...
        return FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            List<WebStorageItem> webStorageItems = snapshot.data ?? <WebStorageItem>[];
            ...
            if (localStorageValueTrackingEdit.length != webStorageItems.length) {
              ...
              localStorageValueTrackingEdit = List.filled(webStorageItems.length, false);
            }
            ...
            rows.addAll(webStorageItems.map((webStorageItem) {
              ...
              return DataRow(cells: <DataCell>[
                _buildDataCellEditable(
                    ...
                    onFieldSubmitted: (newValue) async {
                      var updateItemValue =
                          await webViewController?.webStorage.localStorage.getItem(key: webStorageItem.key!);
                      await webViewController?.webStorage.localStorage.removeItem(key: webStorageItem.key!);
                      await webViewController?.webStorage.localStorage.setItem(key: newValue, value: updateItemValue);
                    },
            ...
            return ExpansionTile(
              ...
              children: <Widget>[
                ...
                _buildAddNewWebStorageItem(
                  ...
                  onAdded: (name, value) {
                    webViewController?.webStorage.localStorage.setItem(key: name, value: value);
                  },
                ),
                SizedBox(
                  width: MediaQuery.of(context).size.width,
        ...
        return FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (sessionStorageValueTrackingEdit.length != webStorageItems.length) {
              sessionStorageKeyTrackingEdit = List.filled(webStorageItems.length, false);
              sessionStorageValueTrackingEdit = List.filled(webStorageItems.length, false);
            }
            ...
            rows.addAll(webStorageItems.map((webStorageItem) {
              ...
              return DataRow(cells: <DataCell>[
                _buildDataCellEditable(
                    ...
                    onFieldSubmitted: (newValue) async {
                      var updateItemValue =
                          await webViewController?.webStorage.sessionStorage.getItem(key: webStorageItem.key!);
                      await webViewController?.webStorage.sessionStorage.removeItem(key: webStorageItem.key!);
                      await webViewController?.webStorage.sessionStorage.setItem(key: newValue, value: updateItemValue);
                    },
                    initialValue: webStorageItem.key!,
                    index: index,
              ...
              children: <Widget>[
                ...
                _buildAddNewWebStorageItem(
                  ...
                  onAdded: (name, value) async {
                    await webViewController?.webStorage.sessionStorage.setItem(key: name, value: value);
                    setState(() {});
                  },
                ),
                SizedBox(
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        List<WebsiteDataRecord> dataRecords =
            snapshot.hasData ? (snapshot.data as List<WebsiteDataRecord>) : <WebsiteDataRecord>[];
        ...
        rows.addAll(dataRecords.map((dataRecord) {
          ...
          return DataRow(cells: <DataCell>[
            DataCell(
              SizedBox(
                width: constraints.maxWidth / 3,
                child: Text(dataRecord.displayName ?? "", style: textStyle, softWrap: true),
              ),
              onTap: () {
            ...
            DataCell(
              SizedBox(
                width: constraints.maxWidth / 3,
                child: Text(dataRecord.dataTypes?.join(", ") ?? "", style: textStyle, softWrap: true),
              ),
              onTap: () {
                showDialog(
                  context: context,
            ...
            DataCell(IconButton(
              ...
              onPressed: () async {
                if (dataRecord.dataTypes != null) {
                  await _webStorageManager.removeDataFor(dataTypes: dataRecord.dataTypes!, dataRecords: [dataRecord]);
                }
                setState(() {});
              },
            ))
  ...
  Widget _buildHttpAuthCredentialDatabaseExpansionTile(BoxConstraints constraints) {
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        List<URLProtectionSpaceHttpAuthCredentials> protectionSpaceHttpAuthCredentials =
        ...
        for (var protectionSpaceHttpAuthCredential in protectionSpaceHttpAuthCredentials) {
          var protectionSpace = protectionSpaceHttpAuthCredential.protectionSpace;
          var rows = <DataRow>[];

          if (protectionSpaceHttpAuthCredential.credentials != null) {
            rows.addAll(protectionSpaceHttpAuthCredential.credentials!.map((httpAuthCredential) {
              return DataRow(cells: <DataCell>[
                DataCell(
                  SizedBox(
                    width: constraints.maxWidth / 3,
                    child: Text(httpAuthCredential.username ?? "", style: textStyle, softWrap: true),
                  ),
                  onTap: () {
                    Clipboard.setData(ClipboardData(text: httpAuthCredential.username!));
                  },
                ),
                DataCell(
                  SizedBox(
                    width: constraints.maxWidth / 3,
                    child: Text(httpAuthCredential.password ?? "", style: textStyle, softWrap: true),
                  ),
                  onTap: () {
                    Clipboard.setData(ClipboardData(text: httpAuthCredential.password!));
                  },
                ),
                DataCell(IconButton(
                  icon: const Icon(Icons.cancel),
                  onPressed: () async {
                    if (protectionSpace != null) {
                      await _httpAuthCredentialDatabase.removeHttpAuthCredential(
                          protectionSpace: protectionSpace, credential: httpAuthCredential);
                    }
                    setState(() {});
                  },
                ))
