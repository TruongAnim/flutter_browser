...
import 'package:test_browser_2/webview_tab.dart';
...
class LongPressAlertDialog extends StatefulWidget {
  ...
  const LongPressAlertDialog(
      {Key? key, required this.webViewModel, required this.hitTestResult, this.requestFocusNodeHrefResult})
      : super(key: key);
...
class _LongPressAlertDialogState extends State<LongPressAlertDialog> {
  ...
  Widget _buildOpenNewIncognitoTab() {
    ...
    return ListTile(
      ...
      onTap: () {
        browserModel.addTab(WebViewTab(
          key: GlobalKey(),
          webViewModel: WebViewModel(url: widget.requestFocusNodeHrefResult?.url, isIncognitoMode: true),
        ));
        Navigator.pop(context);
      },
    );
  ...
  Widget _buildCopyAddressLink() {
    return ListTile(
      ...
      onTap: () {
        Clipboard.setData(
            ClipboardData(text: widget.requestFocusNodeHrefResult?.url.toString() ?? widget.hitTestResult.extra!));
        Navigator.pop(context);
      },
    );
  }
  ...
  Widget _buildImageTile() {
    return ListTile(
      contentPadding: const EdgeInsets.only(left: 15.0, top: 15.0, right: 15.0, bottom: 5.0),
      leading: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: <Widget>[
          // CachedNetworkImage(
  ...
  Widget _buildOpenImageNewTab() {
    ...
    return ListTile(
      ...
      onTap: () {
        browserModel.addTab(WebViewTab(
          key: GlobalKey(),
          webViewModel: WebViewModel(url: WebUri(widget.hitTestResult.extra ?? "about:blank")),
        ));
        Navigator.pop(context);
      },
    );
  ...
  Widget _buildSearchImageOnGoogle() {
    ...
    return ListTile(
      ...
      onTap: () {
        if (widget.hitTestResult.extra != null) {
          var url = "http://images.google.com/searchbyimage?image_url=${widget.hitTestResult.extra!}";
          browserModel.addTab(WebViewTab(
            key: GlobalKey(),
